...
const FormGroup: React.FC<IFormGroupProps> = ({ type='text', inputClass='', labelText, error='', isRequired=false, register, isPassword=false, className='', isDisabled=false, tooltipText='', tooltipContent='', onBlur, readonly=false }) => {
  return (
    <Label tooltipText={ tooltipText } type={type} tooltipContent={ tooltipContent } required={ isRequired } className={`w-full break-words text-wrap flex flex-col items-start justify-start font-bold text-sm ${ className }`} hasError={ Boolean(error) } labelText={ labelText }>
